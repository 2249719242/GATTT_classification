...
# Import Components (Reuse Geometry components)
from GATTT import TemporalAttentionPooling, grad_reverse, tangent_space_projection

# ==========================================
# Spatiotemporal Transformer Block (LSTMA style)
# ==========================================
class STTransformerLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # 1. Spatial Attention (Shared across Time)
        ...
        # 2. Temporal Attention (Shared across Nodes)
        # Permute to (B, N, T, D) then Merge Batch and Nodes: (B*N, T, D)
        x_t = x.permute(0, 2, 1, 3).reshape(B*N, T, D)
        attn_out, _ = self.temporal_attn(x_t, x_t, x_t)
        x_t = self.norm2(x_t + self.dropout2(attn_out))
        # Reshape back to (B, N, T, D) -> (B, T, N, D)
...
class LSTMAGeoModel(nn.Module):
    def __init__(self, num_nodes=200, time_steps=80, num_sites=17, hidden_dim=48):
        super(LSTMAGeoModel, self).__init__()
        
        # --- Stream 1: LSTMA (Spatiotemporal Transformer) ---
        # Input Embedding: 1 -> Hidden
        self.input_emb = nn.Linear(1, hidden_dim)
        
        # ST-Transformer Layers
        self.layer1 = STTransformerLayer(hidden_dim, nhead=4, dropout=0.6)
        self.layer2 = STTransformerLayer(hidden_dim, nhead=4, dropout=0.6)
    ...
    def forward(self, x, alpha=1.0):
        ...
        # --- Stream 1: LSTMA ---
        # (B, T, N) -> (B, T, N, 1) -> (B, T, N, D)
        x_st = self.input_emb(x.unsqueeze(-1))
        
        x_st = self.layer1(x_st)
        x_st = self.layer2(x_st) # (B, T, N, D)
        
        # Global Pooling over Nodes first
        x_st = torch.mean(x_st, dim=2) # (B, T, D)
        # Then Temporal Attention Pooling
        feat_st = self.temp_pool(x_st) # (B, D)
        
        # --- Stream 2: Geometry ---
        feat_geo_raw = tangent_space_projection(x)
        feat_geo = self.tangent_net(feat_geo_raw) # (B, D)
        
        # --- Fusion ---
        feat_fused = torch.cat([feat_st, feat_geo], dim=1) # (B, 2D)
...
def main():
    ...
    print("Loading data for LSTMA + Geometry Model...")
    ...
    for fold, (train_idx, val_idx) in enumerate(gkf.split(X, y, groups)):
        ...
        model = LSTMAGeoModel(num_nodes=200, time_steps=80, num_sites=num_sites, hidden_dim=48).to(device)
        # Use existing robust training hyperparameters
    ...
    df.loc['Mean'] = df.mean()
    df.loc['Std'] = df.std()
    df.to_csv('5_fold_results_lstma_geo.csv')
    print("Saved to 5_fold_results_lstma_geo.csv")
