...
# ==========================================
# LSTMA Block (Reused from train_LSTMA_Geo.py)
# ==========================================
...
# ==========================================
# GCN + LSTMA + Geometry (Flagship Model)
# ==========================================
class GCNLSTMAGeoModel(nn.Module):
    def __init__(self, adj, num_nodes=200, time_steps=80, num_sites=17, hidden_dim=48):
        super(GCNLSTMAGeoModel, self).__init__()
        ...
        # --- Stream 1.5: LSTMA (replacing Standard Transformer) ---
        # No need for feature_bottleneck or spatial_proj here, as LSTMA handles (B, T, N, D) directly
        # But we need to ensure dim matches hidden_dim
    ...
    def forward(self, x, alpha=1.0):
        ...
        x_gcn = self.st_gcn2(x_gcn, adj_expanded) # (B, Hidden, T, N)
        
        # Prepare for LSTMA: Need (B, T, N, Hidden)
        x_gcn = x_gcn.permute(0, 2, 3, 1) # (B, T, N, Hidden)
        
        # --- 2. LSTMA Stage ---
        x_lstma = self.lstma_layer1(x_gcn)
        x_lstma = self.lstma_layer2(x_lstma) # (B, T, N, Hidden)
        
        # Pooling: First mean over nodes, then attention over time
...
def validate_subject_wise(model, X, y, groups, device, batch_size=32):
    ...
    for sub_id in unique_groups:
        ...
        try:
            ...
            # Handle edge case where a fold split might separate windows of a subject??
            # Should not happen with GroupKFold but safe to ignore
            continue 
...
def main():
    ...
    print("Loading data for GCN + LSTMA + Geo (Flagship Model)...")
    ...
    for fold, (train_idx, val_idx) in enumerate(gkf.split(X, y, groups)):
        ...
        model = GCNLSTMAGeoModel(adj=compute_adj_matrix(X_train, top_k=15).to(device), 
    ...
    df.loc['Mean'] = df.mean()
    df.loc['Std'] = df.std()
    df.to_csv('5_fold_results_gcn_lstma_geo.csv')
    print("Saved to 5_fold_results_gcn_lstma_geo.csv")
