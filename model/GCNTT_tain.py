...
# 导入 GCN + Transformer + DANN 模型
# 假设文件名为 GCNTT.py，类名为 ASDClassifier
from GCNTT import ASDClassifier, compute_adj_matrix
...
def validate_subject_wise(model, X, y, groups, device, batch_size=32):
    ...
    with torch.no_grad():
        for batch_x, _ in loader:
            ...
            # GCNTT 返回 (class_logits, site_logits)
...
def main():
    ...
    # 2. 加载数据
    print("正在加载数据 (GCN+Transformer Baseline)...")
    if not os.path.exists('X_augmented.npy'):
        print("错误: 找不到数据文件 (X_augmented.npy 等)。")
        return
    ...
    print(f"\n{'='*20} GCN+Transformer (Baseline) 5-Fold CV {'='*20}")
    ...
    for fold, (train_idx, val_idx) in enumerate(gkf.split(X, y, groups)):
        ...
        for epoch in range(EPOCHS):
            ...
            for i, (batch_x, batch_y, batch_sites) in enumerate(train_loader):
                ...
                # Forward (GCNTT only returns 2 values)
                ...
                # Loss = Class + DANN (No geometry loss)
                loss = (loss_class + 0.05 * loss_site) / ACCUM_STEPS
            ...
            if val_metrics['acc'] > best_fold_metrics['acc']:
                ...
                patience_counter = 0 
                # torch.save(model.state_dict(), f'best_gcntt_fold_{fold+1}.pth')
            else:
                patience_counter += 1
    ...
    # 4. 汇总结果
    print(f"\n{'='*20} GCN+Transformer 5-Fold Summary {'='*20}")
    ...
    csv_filename = '5_fold_results_gcntt.csv'
